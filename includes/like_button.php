...
<script>
// Only inject the modal HTML if it doesn't exist yet
...
// Create a namespaced modal manager if it doesn't exist already
if (!window.LoginModalManager) {
    window.LoginModalManager = {
        element: null,
        isOpen: false,
        
        init: function() {
            this.element = document.getElementById('globalLoginModal');
            
            // Only set up event listeners if we haven't already
            if (this.element && !this.element.dataset.initialized) {
                // Close button
                ...
                // ESC key (only add once)
                if (!window.loginModalEscHandler) {
                    window.loginModalEscHandler = true;
...
if (typeof window.toggleLike !== 'function') {
    window.toggleLike = function(event, button, songId) {
        event.preventDefault();
        event.stopPropagation();
        
        // If user is not logged in or button is disabled
